from odoo import models
from base64 import b64encode, b64decode
from lxml import etree

class PeruSunatCpe(models.Model):
    _inherit = 'facturaclic.cpe'

    def _prepare_cpe(self):
        # Deja que el módulo original genere el XML completo
        res = super()._prepare_cpe()

        # Normalizar la fuente del XML a bytes
        xml_bytes = None
        if self.xml_document:
            # xml_document puede ser str o bytes; normalizamos a bytes
            xml_bytes = self.xml_document if isinstance(self.xml_document, (bytes, bytearray)) else self.xml_document.encode('utf-8')
        elif self.datas:
            # En algunos flujos puede no haber xml_document pero sí datas
            xml_bytes = b64decode(self.datas)

        if not xml_bytes:
            return res  # No hay XML para modificar

        try:
            # Namespaces UBL usados por el generador
            ns = {
                'cac': 'urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2',
                'cbc': 'urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2',
            }

            # Parsear el XML
            doc = etree.fromstring(xml_bytes)

            # Tomar la factura: este CPE siempre tiene invoice_ids[0] para sync
            move = self.invoice_ids and self.invoice_ids[0] or False
            new_code = False
            if move and move.associated_warehouse_id:
                new_code = move.associated_warehouse_id.l10n_pe_edi_address_type_code

            # Si hay código, reemplazar el del emisor (AccountingSupplierParty → PartyLegalEntity → RegistrationAddress)
            if new_code:
                nodes = doc.xpath(
                    './/cac:AccountingSupplierParty//cac:PartyLegalEntity//cac:RegistrationAddress/cbc:AddressTypeCode',
                    namespaces=ns
                )
                for node in nodes:
                    node.text = str(new_code)

                # Serializar y actualizar coherentemente ambos campos
                xml_out = etree.tostring(doc, encoding='UTF-8', xml_declaration=True)
                # Importante: xml_document se usa como str en _sign_cpe (hace .encode('utf-8'))
                self.xml_document = xml_out.decode('utf-8')
                self.datas = b64encode(xml_out)  # mantener datas alineado con xml_document

        except Exception:
            # Defensivo: si falla la edición, no romper el flujo del módulo original
            pass

        return res

